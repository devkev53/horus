...
nitInput.value = client_nit;
// clienteView.val(client_name);
get_client_for_nit(client_nit);
...
// ------------------------ FUNCION PARA TRAER EL LISTADO DE LOS ITEMS DEL DETALLE ------------------------
const get_det = async () => {
  let data = new FormData();
  data.append("action", "search_details");
  data.append("id", sale_id);
...
$("#sale-prods-table tbody")
  ...
  .on("change", 'input[name="quantity"]', function () {
    ...
    let price = sales.items.products[tr.row].price_sale;
    let rowSubtotal = (price * sales.items.products[tr.row].quantity).toFixed(
      2
    );
